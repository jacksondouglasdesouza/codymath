...
describe("toWordsPure", () => {
   ...
   it("should handle large scale numbers correctly up to the safe limit", () => {
      ...
      expect(toWordsPure(123456789)).toBe(
   ...
   it("should handle edge cases and invalid inputs gracefully", () => {
      expect(toWordsPure(Number.MAX_SAFE_INTEGER)).toBe(
         "nine quadrillion seven trillion one hundred ninety nine billion two hundred fifty four million seven hundred forty thousand nine hundred ninety one",
      );
      expect(toWordsPure(Number.MAX_SAFE_INTEGER + 1)).toBe(
         "Integer part is too large or not a safe integer.",
      );
      expect(toWordsPure(-Number.MAX_SAFE_INTEGER - 1));
      expect(toWordsPure(10, "xx-XX")).toBe(
         "Language 'xx-XX' is not supported.",
      );
...
describe("toWordsCurrency", () => {
   it("should convert numbers correctly with default options (USD)", () => {
      ...
      expect(toWordsCurrency(123.45)).toBe(
         "one hundred twenty three dollars and forty five cents",
      );
      expect(toWordsCurrency(10.0)).toBe("ten dollars");
      expect(toWordsCurrency(0.5)).toBe("fifty cents");
      expect(toWordsCurrency(45.678)).toBe(
         "forty five dollars and sixty eight cents",
      );
   });
   ...
   it("should handle singular vs. plural currency names correctly", () => {
      ...
      expect(toWordsCurrency(2.5)).toBe("two dollars and fifty cents");
   });
   ...
   it("should work correctly with custom currency options", () => {
      ...
      expect(toWordsCurrency(123.45, brlOptions)).toBe(
         "one hundred twenty three reais and forty five centavos",
      );
      expect(toWordsCurrency(1.01, brlOptions)).toBe(
         "one real and one centavo",
      );
      expect(toWordsCurrency(2, brlOptions)).toBe("two reais");
   });

   it("should handle negative numbers correctly", () => {
      expect(toWordsCurrency(-10.5)).toBe("minus ten dollars and fifty cents");
   ...
   it("should handle large numbers and safe integer limits", () => {
      expect(toWordsCurrency(1_234_567.89)).toBe(
         "one million two hundred thirty four thousand five hundred sixty seven dollars and eighty nine cents",
      );

      expect(
         toWordsCurrency(Number.MAX_SAFE_INTEGER / 100).length,
      ).toBeGreaterThan(50);

      expect(toWordsCurrency(Number.MAX_SAFE_INTEGER + 1)).toBe(
         "Integer part is too large or not a safe integer.",
      );
   });
...
describe("toWordsPure and toWordsCurrency for pt-BR", () => {
   describe("toWordsPure (pt-BR)", () => {
      ...
      it('should handle decimals with "vírgula" by spelling digits individually', () => {
         expect(toWordsPure(123.45, "pt-BR")).toBe(
            "cento e vinte e três vírgula quatro cinco",
         );
         expect(toWordsPure(120.01, "pt-BR")).toBe(
            "cento e vinte vírgula zero um",
         );
         expect(toWordsPure(999.99, "pt-BR")).toBe(
            "novecentos e noventa e nove vírgula nove nove",
         );
      });
   ...
   describe("toWordsCurrency (pt-BR)", () => {
      ...
      it("should convert varied currency values correctly", () => {
         ...
         expect(toWordsCurrency(0.5, brlOptions)).toBe("cinquenta centavos");
         expect(toWordsCurrency(25.5, brlOptions)).toBe(
            "vinte e cinco reais e cinquenta centavos",
         );
         expect(toWordsCurrency(999.99, brlOptions)).toBe(
            "novecentos e noventa e nove reais e noventa e nove centavos",
         );
         expect(toWordsCurrency(1200000, brlOptions)).toBe(
      ...
      it("should handle singular vs. plural for BRL correctly", () => {
         ...
         expect(toWordsCurrency(2.01, brlOptions)).toBe(
      ...
      it("should handle rounding correctly for 3 and 4 decimal places", () => {
         ...
         expect(toWordsCurrency(10.554, brlOptions)).toBe(
            "dez reais e cinquenta e cinco centavos",
         );
         // Arredondamento para cima
         expect(toWordsCurrency(10.555, brlOptions)).toBe(
            "dez reais e cinquenta e seis centavos",
         );
      ...
      it("should handle negative currency values correctly", () => {
         ...
         expect(toWordsCurrency(-0.5, brlOptions)).toBe(
            "menos cinquenta centavos",
         );
         expect(toWordsCurrency(-125.45, brlOptions)).toBe(
            "menos cento e vinte e cinco reais e quarenta e cinco centavos",
         );
      });
      ...
      it("should perform consistently under a brutal stress test", () => {
         ...
         for (let i = 0; i < iterations; i++) {
            ...
            const roundedNum = divide(
               round(multiply(absolute(randomNum), 100)),
               100,
            );
            const integerPart = floor(roundedNum);
            const fractionalPart = round(
               multiply(subtract(roundedNum, integerPart), 100),
            );
...
describe("toWordsPure (es-ES)", () => {
   ...
   it('should handle decimals with "coma" by spelling digits individually', () => {
      ...
      expect(toWordsPure(99.99, "es-ES")).toBe(
         "noventa y nueve coma nueve nueve",
      );
   });

   it('should handle negative numbers with "menos"', () => {
      expect(toWordsPure(-10, "es-ES")).toBe("menos diez");
      expect(toWordsPure(-1500.25, "es-ES")).toBe(
         "menos mil quinientos coma dos cinco",
      );
   });
...
describe("toWordsCurrency (es-ES)", () => {
   ...
   it("should convert currency correctly with default Spanish options (Euros)", () => {
      ...
      expect(toWordsCurrency(25.5, esOptions)).toBe(
         "veinticinco euros con cincuenta céntimos",
      );
   ...
   it("should handle rounding correctly", () => {
      expect(toWordsCurrency(10.555, esOptions)).toBe(
   ...
   it("should work correctly with custom currency names in Spanish", () => {
      ...
      expect(toWordsCurrency(5.5, customOptions)).toBe(
         "cinco dólares con cincuenta centavos",
      );
      expect(toWordsCurrency(1.01, customOptions)).toBe(
...
describe("getIntegerPart", () => {
   it("should return the integer itself for whole numbers within the safe range", () => {
      ...
      expect(getIntegerPart(Number.MAX_SAFE_INTEGER)).toBe(
   ...
   it("should return the integer part of floating-point numbers within the safe range", () => {
      ...
      expect(getIntegerPart(Number.MAX_SAFE_INTEGER - 0.5)).toBe(
   ...
   it("should throw an error for numbers GREATER than the safe integer range", () => {
      ...
      expect(() => getIntegerPart(unsafeBigNumber)).toThrow(
   ...
   it("should throw an error for numbers LESS than the safe integer range", () => {
      ...
      expect(() => getIntegerPart(unsafeSmallNumber)).toThrow(
         "Input number is outside the safe integer range and may cause precision loss.",
      );
   });
...
describe("countDigitsDetailed", () => {
   ...
   it("should correctly count digits for positive floating-point numbers", () => {
      expect(countDigitsDetailed(123.45)).toEqual({
         integer: 3,
         fractional: 2,
      });
      expect(countDigitsDetailed(0.12345)).toEqual({
         integer: 1,
         fractional: 5,
      });
      expect(countDigitsDetailed(98765.4)).toEqual({
         integer: 5,
         fractional: 1,
      });
   });
   ...
   it("should correctly count digits for negative floating-point numbers", () => {
      ...
      expect(countDigitsDetailed(-0.987)).toEqual({
         integer: 1,
         fractional: 3,
      });
   });
   ...
   it("should perform consistently under a brutal stress test", () => {
      ...
      for (let i = 0; i < iterations; i++) {
         ...
         const expectedIntegerCount =
            parts[0] === "0" && parts.length > 1 ? 1 : parts[0].length;
...
describe("padZeroes", () => {
   ...
   it("should perform consistently under a brutal stress test", () => {
      ...
      for (let i = 0; i < iterations; i++) {
         ...
         const expectedPadded = integerPart
            .toString()
